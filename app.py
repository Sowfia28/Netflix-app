 ...
 if st.button('Predict'):
    ...
    if not all([country.strip(), director.strip(), genre.strip()]):
        ...
        with st.spinner('Predicting...'):
            ...
            # Replace unseen inputs with mode from training data
            ...
            if logistic_model_imdb is not None:
                for col in cols_imdb_logistic:
                    if input_df[col].iloc[0] not in features_imdb_log[col].unique():
                        input_df.at[0, col] = features_imdb_log[col].mode(dropna=True)[0]
